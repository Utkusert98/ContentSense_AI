...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    // GÜNCELLENDİ: Sınırsız yerine gerçek rakamları gösteriyoruz
    ...
    if(confirm(`Emin misiniz? Hesabınız ${targetPlan} paketine geçirilecektir.`)) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // GÜNCELLENDİ: Gelen gerçek krediyi state'e ve hafızaya al
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-1 flex flex-col gap-2">
            <button 
              ...
              className={`text-left px-5 py-3 rounded-xl font-bold transition-all ${activeTab === "profil" ? 'bg-[var(--color-primary)] text-white shadow-md' : 'text-gray-600 hover:bg-white/60'}`}
            >
              👤 Profil Bilgileri
            </button>
            <button 
              onClick={() => setActiveTab("abonelik")}
              className={`text-left px-5 py-3 rounded-xl font-bold transition-all ${activeTab === "abonelik" ? 'bg-[var(--color-primary)] text-white shadow-md' : 'text-gray-600 hover:bg-white/60'}`}
            >
              💳 Abonelik & Kredi
            </button>
            <button 
              onClick={() => setActiveTab("sifre")}
              className={`text-left px-5 py-3 rounded-xl font-bold transition-all ${activeTab === "sifre" ? 'bg-[var(--color-primary)] text-white shadow-md' : 'text-gray-600 hover:bg-white/60'}`}
            >
              🔒 Şifre Değiştir
            </button>
          </div>
          ...
          <div className="md:col-span-3 glass-panel p-8 min-h-[400px]">
            ...
            {activeTab === "profil" && (
              <form onSubmit={handleSaveProfile} className="space-y-6 animate-fade-in">
                  ...
                  <div className="flex flex-wrap gap-3">
                      {availableCategories.map((cat) => (
                          <button 
                            ...
                            className={`px-5 py-3 rounded-full font-medium transition-all duration-200 border text-sm ${categories.includes(cat) ? 'bg-[var(--color-primary)] text-white border-[var(--color-primary)] shadow-md hover:opacity-90' : 'bg-white/50 text-gray-700 border-gray-300 hover:bg-white'}`}
                          >
                              {cat}
                          </button>
                      ))}
                ...
                <div className="pt-6">
                  <button type="submit" disabled={isLoading} className="bg-gray-900 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-gray-800 transition-all hover:scale-105 disabled:opacity-50">
            ...
            {activeTab === "abonelik" && (
              <div className="animate-fade-in">
                ...
                <div className={`mb-10 p-8 rounded-3xl border flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-white transition-all
                    ${plan === 'Gold' ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 border-yellow-300 shadow-xl shadow-yellow-400/30' : 
                      plan === 'Pro' ? 'bg-[var(--color-primary)] border-blue-400 shadow-xl shadow-blue-400/30' : 
                      'bg-gray-800 border-gray-700 shadow-xl shadow-gray-900/30'}`}
                >
                  <div className="flex-1">
                    ...
                    {/* GÜNCELLENDİ: Gerçekçi Plan Özellikleri */}
                    <ul className="text-sm space-y-2 opacity-90 mb-6">
                      {plan === "Standart" && (
                        <>
                          <li className="flex items-center gap-2"><span>✔️</span> Ayda 3 İçerik Kredisi</li>
                          <li className="flex items-center gap-2"><span>✔️</span> Standart Çözünürlük (720p)</li>
                          <li className="flex items-center gap-2 text-gray-400"><span className="opacity-50">❌</span> Filigranlı Çıktı</li>
                        </>
                      )}
                      {plan === "Pro" && (
                        <>
                          <li className="flex items-center gap-2"><span>✔️</span> Ayda 50 İçerik Kredisi</li>
                          <li className="flex items-center gap-2"><span>✔️</span> Yüksek Çözünürlük (1080p)</li>
                          <li className="flex items-center gap-2"><span>✔️</span> Öncelikli YZ Hızı & Filigransız</li>
                        </>
                      )}
                      {plan === "Gold" && (
                        <>
                          <li className="flex items-center gap-2"><span>✔️</span> Ayda 200 İçerik Kredisi</li>
                          <li className="flex items-center gap-2"><span>✔️</span> Ultra Çözünürlük (4K) & Marka Kiti</li>
                          <li className="flex items-center gap-2"><span>✔️</span> Özel Destek & Ekip Yönetimi</li>
                        </>
                      )}
                    </ul>
                    ...
                    <div className="inline-block bg-black/20 px-5 py-2.5 rounded-xl border border-white/10">
                      <p className="text-base flex items-center gap-2">
                        <span className="text-yellow-300 text-xl">⚡</span> Kalan Kredi: <strong className="text-xl">{credits}</strong>
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className={`p-6 rounded-2xl border transition-all duration-300 flex flex-col ${plan === "Standart" ? 'bg-gray-50 border-gray-300 opacity-60' : 'bg-white border-gray-300 hover:border-gray-400'}`}>
                        <h4 className="font-bold text-xl text-gray-800">Standart</h4>
                        <p className="text-sm text-gray-500 mb-4">Ücretsiz Deneme</p>
                        <ul className="text-sm text-gray-600 space-y-3 flex-grow">
                            <li className="flex items-center gap-2"><span>✔️</span> 3 Kredi / Ay</li>
                            <li className="flex items-center gap-2"><span>✔️</span> 720p Çözünürlük</li>
                            <li className="flex items-center gap-2 text-gray-400"><span className="opacity-50">❌</span> Filigranlı İndirme</li>
                        </ul>
                        <div className="mt-6">
                            {plan === "Standart" ? (
                                <button disabled className="w-full bg-gray-200 text-gray-500 py-3 rounded-xl font-bold cursor-not-allowed">Mevcut Plan</button>
                            ) : (
                                <button onClick={() => handleUpgradePlan("Standart")} disabled={isLoading} className="w-full bg-gray-900 text-white py-3 rounded-xl font-bold hover:bg-gray-800 transition-colors shadow-md disabled:opacity-50">Seç</button>
                            )}
                        </div>
                    </div>
                    ...
                    <div className={`p-6 rounded-2xl border transition-all duration-300 flex flex-col ${plan === "Pro" ? 'bg-blue-50 border-[var(--color-primary)] ring-2 ring-[var(--color-primary)]/40 shadow-md' : 'bg-white border-gray-300 hover:border-[var(--color-primary)]'}`}>
                        <h4 className="font-bold text-xl text-gray-800">Pro</h4>
                        <p className="text-sm text-[var(--color-primary)] font-bold mb-4">₺199 / Ay</p>
                        <ul className="text-sm text-gray-600 space-y-3 flex-grow">
                            <li className="flex items-center gap-2"><span>✔️</span> 50 Kredi / Ay</li>
                            <li className="flex items-center gap-2"><span>✔️</span> 1080p Çözünürlük</li>
                        ...
                        <div className="mt-6">
                            {plan === "Pro" ? (
                                <button disabled className="w-full bg-[var(--color-primary)] text-white py-3 rounded-xl font-bold cursor-not-allowed opacity-80">Mevcut Plan</button>
                            ) : (
                                <button onClick={() => handleUpgradePlan("Pro")} disabled={isLoading} className="w-full bg-[var(--color-primary)] text-white py-3 rounded-xl font-bold hover:bg-blue-600 transition-colors shadow-md disabled:opacity-50">Yükselt</button>
                            )}
                        </div>
                    </div>
                    ...
                    <div className={`p-6 rounded-2xl border transition-all duration-300 flex flex-col ${plan === "Gold" ? 'bg-yellow-50 border-yellow-400 ring-2 ring-yellow-400/50 shadow-md' : 'bg-white border-gray-300 hover:border-yellow-500'}`}>
                        <h4 className="font-bold text-xl text-gray-800">Gold</h4>
                        <p className="text-sm text-yellow-600 font-bold mb-4">₺499 / Ay</p>
                        <ul className="text-sm text-gray-600 space-y-3 flex-grow">
                            <li className="flex items-center gap-2"><span>✔️</span> 200 Kredi / Ay</li>
                            <li className="flex items-center gap-2"><span>✔️</span> 4K Ultra Çözünürlük</li>
                            <li className="flex items-center gap-2"><span>✔️</span> Marka Kiti (Logo/Font)</li>
                            <li className="flex items-center gap-2"><span>✔️</span> Ekip Yönetimi & API</li>
                        </ul>
                        <div className="mt-6">
                            {plan === "Gold" ? (
                                <button disabled className="w-full bg-yellow-500 text-white py-3 rounded-xl font-bold cursor-not-allowed opacity-80">Mevcut Plan</button>
                            ) : (
                                <button onClick={() => handleUpgradePlan("Gold")} disabled={isLoading} className="w-full bg-yellow-500 text-white py-3 rounded-xl font-bold hover:bg-yellow-600 transition-colors shadow-md disabled:opacity-50">Yükselt</button>
                            )}
                        </div>
                    </div>
            ...
            {activeTab === "sifre" && (
              <form onSubmit={handleChangePassword} className="space-y-6 animate-fade-in">
                ...
                <div className="pt-4">
                  <button type="submit" disabled={isLoading} className="bg-[var(--color-primary)] text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-blue-600 transition-all hover:scale-105 disabled:opacity-50">
