...
export default function DashboardNavbar() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("creditsUpdated", checkCredits);
  }, []);
  ...
  // GÜNCELLENDİ: Yorum satırı hatası çözüldü
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/85 backdrop-blur-2xl border-b border-white/50 shadow-sm px-6 py-4 flex items-center justify-between transition-all">
      ...
      <div className="flex gap-4 items-center">
        <div className="bg-white/60 px-4 py-2 rounded-full border border-gray-200 flex items-center gap-2 shadow-sm font-semibold text-sm text-gray-700">
          <span className="text-yellow-500 text-lg">⚡</span> 
          {plan === "Standart" ? `${credits} Kredi Kaldı` : "Sınırsız Kredi"}
        </div>
